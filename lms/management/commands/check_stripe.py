...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not settings.STRIPE_SECRET_KEY:
            self.stdout.write(
                self.style.ERROR("❌ STRIPE_SECRET_KEY не установлен в настройках")
            )
        ...
        if not settings.STRIPE_PUBLISHABLE_KEY:
            self.stdout.write(
                self.style.WARNING("⚠️  STRIPE_PUBLISHABLE_KEY не установлен")
            )
        ...
        try:
            ...
            if settings.STRIPE_PUBLISHABLE_KEY:
                self.stdout.write(
                    f"   Publishable Key: {settings.STRIPE_PUBLISHABLE_KEY[:10]}..."
                )

        except stripe.error.AuthenticationError as e:
            self.stdout.write(self.style.ERROR(f"❌ Ошибка аутентификации Stripe: {e}"))
            self.stdout.write(
                "   Проверьте правильность STRIPE_SECRET_KEY в .env файле"
            )
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"❌ Ошибка при подключении к Stripe: {e}")
            )

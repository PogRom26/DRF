...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action == "create":
            ...
            ]  # Просмотр только авторизованным
        else:  # 'list'
            # Список пользователей доступен только админам и модераторам
            permission_classes = [
                permissions.IsAuthenticated,
                permissions.IsAdminUser | IsNotModerator,
            ]
        ...
        if payment.payment_method == "stripe":
            try:
                ...
                if payment.course:
                    ...
                    raise ValueError(
                        "Для оплаты через Stripe необходимо указать курс или урок"
                    )
                ...
                logger.error(
                    f"Error integrating Stripe for payment {payment.id}: {str(e)}"
                )
                ...
                return Response(
                    {
                        "error": session_result.get(
                            "error", "Ошибка при проверке статуса Stripe"
                        )
                    },
...
class CreateStripePaymentForCourseAPIView(APIView):
    ...
    def post(self, request, course_id):
        ...
        existing_payment = Payment.objects.filter(
            user=user, course=course, payment_status="succeeded"
        ).first()
        ...
        try:
            ...
            if not stripe_result["success"]:
                return Response(
                    {
                        "success": False,
                        "error": stripe_result.get(
                            "error", "Ошибка при создании платежа Stripe"
                        ),
                    },
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )
